...
CnCMap::CnCMap() {
  ...
  this->triggerPool = new std::vector<Trigger*>();
}
 ...
 */
void CnCMap::loadMap(const std::string& mapname, LoadingScreen* lscreen) {
  // Copy the map name
...
Uint8 CnCMap::absScroll(Sint16 dx, Sint16 dy, Uint8 border)
{
  ...
  if (dx <= -border)
  {
    ...
    if (validx)
    {
      scrollvec.x = (Sint8)(std::min(dx, (Sint16)100) * fmax);
      direction |= s_left;
    }
    else
    {
  ...
  if (dy <= -border)
  {
    ...
    if (validy)
    {
      scrollvec.y = (Sint8)(std::min(dy, (Sint16)100) * fmax);
      direction |= s_up;
    }
    else
    {
...
SDL_Surface* CnCMap::getMiniMap(Uint8 pixsize)
{
  ...
  tx = std::min(miniclip.sidew, (Uint16)minimap->w);
  ty = std::min(tx, (Uint16)minimap->h);
 ...
 */
RA_Teamtype* CnCMap::getTeamtypeByName(std::string teamNameString)
{
...
Uint32 CnCMap::getResourceFrame(Uint32 pos) const {
  if (std::string(missionData->theater).substr(0, 3) == "INT") {
    // no image
    return 0;
  }
...
void CnCMap::loadIni(INIFile* inifile)
{
  ...
  std::string moveflsh = "moveflsh." + missionData->theater.substr(0, 3);
  flashnum = 0;
...
/**
 */
Uint8 CnCMap::UnitActionToNr(const std::string action)
{
  if (action == "Sleep")
  {
...
void CnCMap::advancedSections(INIFile *inifile) {
  ...
  for (int keynum = 0; keynum<numKeys; keynum++) {
    ...
    std::string shpnameTerrain = nameTerrain + "." + missionData->theater.substr(0, 3);
...
void CnCMap::unOverlayPack(INIFile *inifile) {
  ...
  for (int keynum = 1; keynum < numKeys+1; keynum++) {
    ...
    {
      std::string stringReaded = inifile->readString("OVERLAYPACK", tempStrS.str().c_str());
...
/**
 */
void CnCMap::parseOverlay(const Uint32& linenum, const std::string& name) {
  Uint8 type, frame;
  Uint16 res;
      ...
      || name.substr(0, 3) == "GEM")
  {
    ...
    std::map<std::string, Uint8>::iterator t = resourcenames.find(name);
...
SDL_Surface* CnCMap::loadTile(INIFile* templini, Uint16 templ, Uint8 tile, Uint32* tiletype) {
  ...
  *tiletype = templini->readInt(tilename.str(), tilenum, 0);
  ...
  std::string temname = templini->readString(tilename.str(), "NAME");
...
void CnCMap::loadTeamTypes(INIFile* fileIni) {
  ...
  for (keynum = 0; keynum < numberOfKey; keynum++) {
    if (maptype == GAME_RA) {
      ...
      //printf("temp = %s\n", temp.c_str());
      std::string temp3 = std::string(splitList(cppstrdup(temp.c_str()), ',')[0]);
      //printf("temp3 = %s\n", temp3.c_str());
      pos = temp.find(",", 0);
      std::string temp2 = temp.substr(pos + 1, temp.size());
      //printf("temp2 = %s\n", temp2.c_str());
 ...
 */
std::vector<OpenRedAlert::Game::Trigger*>* CnCMap::getTriggerPool() {
