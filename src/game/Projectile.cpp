...
Projectile::Projectile(std::string pname, ProjectileDataList* data, std::vector<SHPImage*>* imagePool) {
  ...
  // Get image name
  std::string lImage = this->lnkProjectileData->getImage();
  ...
  if (lImage!= "none"){
    ...
    try {
      ...
      Logger::getInstance()->Error("Image '" + lImage + "' not found during loading of '" + pname + "' projectile");
    }
    // stack the image
