...
numdeletedstruct(0)
{
  ...
  this->theaterext = "." + std::string(theTheater).substr(0, 3);
...
UnitAndStructurePool::~UnitAndStructurePool() {
  ...
  typedef std::multimap<UnitType*, std::vector<StructureType*>* >::const_iterator Iu;
  typedef std::multimap<StructureType*, std::vector<StructureType*>* >::const_iterator Is;
...
Uint8 UnitAndStructurePool::getL2overlays(Uint16 pos, Uint32 **inumbers, Sint8 **xoffset, Sint8 **yoffset) {
  std::multimap<Uint16, L2Overlay*>::iterator entry;
  L2Overlay* curl2;
...
std::multimap<Uint16, L2Overlay*>::iterator UnitAndStructurePool::addL2overlay(Uint16 cellpos, L2Overlay *ov) {
  std::multimap<Uint16, L2Overlay*>::iterator entry;
  l2pool.insert(std::make_pair(cellpos, ov));
...
void UnitAndStructurePool::removeL2overlay(std::multimap<Uint16, L2Overlay*>::iterator entry) {
  Uint32 cellpos;
...
bool UnitAndStructurePool::createReinforcements(RA_Teamtype* Team) {
  ...
  Uint8 owner = 0;
  std::string name_trigger; // name of the trigger of all unit in team
...
bool UnitAndStructurePool::createStructure(const char* typen, Uint16 cellpos,
                                           unsigned int owner, Uint16 health, Uint8 facing, bool makeanim, std::string trigger_name)
{
  ...
  if (0 == type) {
    Logger::getInstance()->Error("Invalid type '" + std::string(typen) + "'");
    return false;
  }
...
bool UnitAndStructurePool::createStructure(StructureType* type, Uint16 cellpos,
                                           unsigned int owner, Uint16 health,
                                           Uint8 facing, bool makeanim,
 ...
 * Creates a unit
 */
Unit* UnitAndStructurePool::createUnit(const char *typen, Uint16 cellpos,
                                       Uint8 subpos, unsigned int owner,
                                       Uint16 health, Uint8 facing,
                                       Uint8 action, std::string trigger_name)
{
  ...
  if (0 == type) {
    Logger::getInstance()->Error("Invalid type name: '" + std::string(typen) + "'");
    return 0;
  }
 ...
 */
UnitType* UnitAndStructurePool::getUnitTypeByName(const std::string& unitname)
{
  UnitType* type;
  Uint16 typenum;

  std::string uname = unitname;

  //transform(uname.begin(),uname.end(), uname.begin(), toupper);

  std::map<std::string, Uint16>::iterator typeentry = unitname2typenum.find(uname);
...
void UnitAndStructurePool::addPrerequisites(UnitType* unittype)
{
  ...
  std::vector<std::string> prereqs = unittype->getPrereqs();
  ...
  std::vector<StructureType*>* type_prereqs;
  for (unsigned int x = 0; x<prereqs.size(); x++)
  {
    type_prereqs = new std::vector<StructureType*>;
...
void UnitAndStructurePool::addPrerequisites(StructureType* structtype)
{
  ...
  std::vector<std::string> prereqs = structtype->getPrereqs();
 ...
 */
std::vector<std::string> UnitAndStructurePool::getBuildableUnits(Player* pl) {
  std::vector<std::string> retval;
  std::vector<StructureType*> prereqs;
  typedef std::multimap<UnitType*, std::vector<StructureType*>* >::const_iterator I;
  std::pair<I,I> b;
  Uint32 y;
  UnitType* utype;
  bool ivalid;
    ...
    if (utype->getTechLevel() != -1 && utype->getTechLevel() < pl->getTechLevel()) {
      ...
      if (ovalid) {
        ...
        for (y=0; y<utype->getOwners().size(); ++y)
        {
          ...
          std::string sider = utype->getOwners()[y];
 ...
 *
 */
std::vector<std::string> UnitAndStructurePool::getBuildableStructures(Player* pl) {
  std::vector<std::string> retval;
  std::vector<StructureType*> prereqs;
  typedef std::multimap<StructureType*, std::vector<StructureType*>* >::const_iterator I;
  std::pair<I,I> b;
  Uint32 y;
  ...
  for (std::map<std::string, StructureType*>::const_iterator x = structuretypepool.begin();
...
{
  std::map<std::string, Talkback*>::iterator typeentry;
  std::string tname(talkback);
 ...
 *
 */
void UnitAndStructurePool::splitORPreReqs(const std::string& prereqs, std::vector<StructureType*>* type_prereqs)
{
  // If prerequiste are "none" we skip it
  if (prereqs == "none")
  {
