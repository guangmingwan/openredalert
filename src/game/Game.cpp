 ...
 */
void Game::InitializeMap(std::string MapName) {
  /// Copy the game mode
  ...
  switch (gamemode)
  {
      ...
      try
    {
      ...
      std::string briefMovieName = p::ccmap->getMissionData().brief;
      ...
      std::string actionMovieName = p::ccmap->getMissionData().action;
...
void Game::InitializeGameClasses()
{
  ...
  catch (SoundError& error)
  {
    ...
    throw std::runtime_error("Unable to Initialize the sound engine");
  }
  ...
  catch (...)
  {
    ...
    throw std::runtime_error("Unable to Initialize the RA Data loader");
  }
      ...
      if (!unit->IsMoving() && !unit->IsAttacking() && !unit->IsHarvesting()) {
        if (unit->aiCommandList.size()>0) {
          ...
          //if (com->getId() == 3 || com->getId() == 8)
          {
            ...
            unit->move(pos, true);
            std::vector<AiCommand*>::iterator i = unit->aiCommandList.begin();
