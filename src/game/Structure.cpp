...
Structure::Structure(StructureType *type, Uint16 cellpos, unsigned int owner,
                     Uint16 rhealth, Uint8 facing, std::string trigger_name) : UnitOrStructure()
{
  targetcell = cellpos;
  this->type = type;
  ...
  for (unsigned int j = 0; j < type->getDeployWith().size(); j++)
  {
    ...
    {
      std::vector<std::string> depWith = this->type->getDeployWith();
      // Health = 256 --> max
...
Uint16 Structure::getBPos(Uint16 pos) const
{
  ...
  dx = 0;
  if ((pos%mwid) > x) {
    dx = std::min((unsigned)(type->getXsize()-1),(unsigned)((pos%mwid)-x)-1);
  }
  y = cellpos/mwid;
  dy = 0;
  if ((pos/mwid) > y) {
    dy = std::min((unsigned)(type->getYsize()-1),(unsigned)((pos/mwid)-y)-1);
  }
...
bool Structure::is(std::string Name) {
...
std::string Structure::getTriggerName() {
  return TriggerName;
}
