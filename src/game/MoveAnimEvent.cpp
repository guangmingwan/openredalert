...
MoveAnimEvent::MoveAnimEvent(uint32_t p, Unit* un) : UnitAnimEvent(p,un) {
...
void MoveAnimEvent::startMoveOne(bool wasblocked)
{
  ...
  uint8_t loopend=((UnitType*)un->type)->getAnimInfo().loopend;
#endif
...
#ifdef LOOPEND_TURN
  face = ((int8_t)((loopend+1)*(8-path->top())/8))&loopend;
#else
  ...
  if (((UnitType *)un->getType())->isInfantry()) {
...
#ifdef LOOPEND_TURN
    uint8_t curface = (un->getImageNum(0)&loopend);
    uint8_t delta = (abs(curface-face))&loopend;
    //        if( curface != face ) {
    ...
    if( curface != face && un->turnanim1 == NULL) {
      if( ((delta <= (int8_t)((loopend+1)/8)) || (delta >= (int8_t)(loopend*7/8))) || un->getType()->getPType() == UN_PLANE) {
#else
    ...
    void MoveAnimEvent::setRange(uint32_t nr) {
      range = nr;
    }
