...
/**
 */
UnitType::UnitType(const std::string& typeName, INIFile* unitini) :
UnitOrStructureType(),
shpnums(0),
c4(false)
{
  ...
  std::string tmp = unitini->readString(tname, "prerequisites", "");
  Split(prereqs, tmp, ',');
  ...
  // Read primary weapon
  std::string priStr = unitini->readString(tname, "Primary", "");
  if (priStr == "")
  {
  ...
  //
  std::string secStr = unitini->readString(tname, "Secondary", "");
  if (secStr == "")
  {
  ...
  std::string deploytarget = unitini->readString(tname, "deploysto", "");
  ...
  // Read the armor
  std::string charArmor = unitini->readString(tname, "Armour", "");
  if (charArmor == "")
    armour = AC_none;
  else
  {
    if (std::string(charArmor) == "none")
      armour = AC_none;
    else if (std::string(charArmor) == "wood")
      armour = AC_wood;
    else if (std::string(charArmor) == "light")
      armour = AC_light;
    else if (std::string(charArmor) == "heavy")
      armour = AC_heavy;
    else if (std::string(charArmor) == "concrete")
      armour = AC_concrete;
  }
...
std::vector<std::string> UnitType::getOwners() const
{
  return owners;
}
...
std::vector<Uint8> UnitType::getPassengerAllow() const
{
