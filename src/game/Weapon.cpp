...
Weapon::Weapon(const char* wname) {
  std::map<std::string, Projectile*>::iterator projentry;
  std::map<std::string, Warhead*>::iterator wheadentry;
  INIFile * weapini;
  //SHPImage * fireanimtemp;
  //Uint8 additional;
  //Uint8 i;
  std::string projname, warheadname;
  std::string weapname;

  INIFile* rules = 0;

  name = std::string(wname);
  ...
  std::string pname = weapini->readString(wname, "projectile", "");
  if (pname.size() == 0)
  {
    Logger::getInstance()->Warning(
                                   "Unable to find projectile for weapon '" + std::string(wname) + "' in inifile..");
    throw 0;
  }
  projname = (std::string)pname;
  ...
  if (projentry == p::weappool->projectilepool.end() )
  {
    try
    {
      projectile = new Projectile(std::string(pname),
  ...
  std::string whname = weapini->readString(wname, "warhead");
  if (whname.size() == 0)
  {
    Logger::getInstance()->Warning("Unable to find warhead for weapon \"%s\" in inifile..\n" + std::string(wname));
    throw 0;
  }
  warheadname = whname;
  ...
  if (wheadentry == p::weappool->warheadpool.end() )
  {
    ...
    {
      Logger::getInstance()->Warning("Unable to find Warhead '" + whname + "' used for weapon '" + std::string(wname) + "'. Unit using this weapon will be unarmed.");
      throw 0;
    }
  ...
  if (report.size() > 0)
  {
    std::string soundWeap = report;
    soundWeap += std::string(".aud");
...
const std::string Weapon::getChargingSound()const
{
