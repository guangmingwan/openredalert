...
/**
 * @param typeName Basic name of the StructureType (ex: FENC, WEAP ...)
 * @param structini structini
 * @param artini artini
 * @param thext thext
 */
StructureType::StructureType(const std::string& typeName, INIFile* structini, INIFile* artini,
...
UnitOrStructureType()
{
  ...
  {
    Logger::getInstance()->Error("Try to read a non existant ini section " + typeName);
    return;
  }
  ...
  {
    // Read prerequiste
    std::string tmp = structini->readString(typeName, "prerequisites", "");
  ...
  std::string tmp = structini->readString(typeName, "deploywith", "");
  if (tmp.size() > 0)
  {
  ...
  // Get the owners
  std::string ownersString = structini->readString(typeName, "owners", "");
  // Split the List of owners
  Split(owners, ownersString, ',');
  // Custom warning
  if (owners.empty())
  {
    Logger::getInstance()->Warning(typeName + " has no owners");
  }
  ...
  // Read "Charges"
  charges = (structini->readYesNo(std::string(typeName), "Charges", 0) == 1);
  ...
  for (i = 0; i < numshps; i++)
  {
    ...
    if (tmp.size() == 0)
    {
      shpname = std::string(typeName) + ".shp";
    }
    else
    {
      shpname = tmp;// + ".shp";
    ...
    {
      shpname = std::string(typeName) + std::string(thext);
      try
      {
        shpimage = new SHPImage(shpname.c_str(), -1);
      }
  ...
  if (!is_wall)
  {
    ...
    std::string shpMakeName;
    // Check if name of the structure <= 4
    if (typeName.size() <= 4)
    {
      shpMakeName = std::string(typeName) + "make.shp";
    }
    else
    {
      Logger::getInstance()->Warning(typeName + " is nonstandard! (name lenght > 4)");
    }
    ...
    std::string strPri = structini->readString(typeName, "primary_weapon", "");
    if (strPri == "")
    {
    ...
    std::string strSec = structini->readString(typeName, "secondary_weapon", "");
    if (strSec == "")
    {
  ...
  std::string armStr = structini->readString(typeName, "armour", "none");
  if (armStr == "none")
  {
    armour = AC_none;
  }
...
std::vector<std::string> StructureType::getDeployWith() const
{
  return deploywith;
}

std::vector<std::string> StructureType::getOwners() const
{
  return owners;
}
