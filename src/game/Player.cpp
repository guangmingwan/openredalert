...
Player::Player(const std::string& pname) {
  ...
  mapVisible= new std::vector<bool>(128*128);
  mapBuildable= new std::vector<bool>(128*128);
}
...
void Player::pauseBuilding() {
  std::map<Uint8, BQueue*>::iterator i;
...
BQueue* Player::getQueue(Uint8 queuenum) {
  std::map<Uint8, BQueue*>::iterator it = queues.find(queuenum);
  if (queues.end() == it) {
    return 0;
  }
  return it->second;
  ...
  if (st->primarySettable() /*&& (st->getPType() != 0)*/) {
    ...
    if (str == getPrimary(str->getType())) {
      if (prg.empty()) {
        ...
        std::map<Uint8, BQueue*>::iterator it = queues.find(st->getPType());
...
std::vector<bool>* Player::getMapVis() {
  return mapVisible;
}

std::vector<bool>* Player::getMapBuildable() {
  return mapBuildable;
}
