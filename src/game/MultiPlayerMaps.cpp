...
bool MultiPlayerMaps::getMapDescription (unsigned int Index, std::string & description)
{
...
bool MultiPlayerMaps::getMapName(unsigned int Index, std::string & Name)
{
...
void MultiPlayerMaps::readMapData() {
  ...
  {
    MACRO_LOG_DEBUG("Line = " + std::string(Line))
    ...
    if ((pos = tmpString.find ("=",0)) != std::string::npos){
      if ((pos1 = tmpString.find (")",0)) != std::string::npos){
        //tmpString.erase (pos, pos+3);
        if (pos < tmpString.size()){
