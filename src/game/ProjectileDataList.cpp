...
/**
 */
ProjectileData* ProjectileDataList::getData(std::string name) {
  std::map<std::string, ProjectileData * >::iterator itRecherche;
  ...
  Logger::getInstance()->Error("PROJECTILEDATA [" + name + "] not found in DataList !!!");

  // Return new ProjectileData by default
  Logger::getInstance()->Warning("New virtual ProjectileData was returned...");
  return new ProjectileData();
}

/**
 */
void ProjectileDataList::loadProjectileData(INIFile* file, std::string name) {
