...
Menu::Menu() : StartNewGameButton()
{
  ...
  MACRO_LOG_DEBUG("Loading background graphics.")
  try {
  ...
  if (win95_logo == NULL){
    ...
    try {
      dos_logo = new CPSImage(std::string("title.cps"), 1);
    } catch (ImageNotFound&) {
      Logger::getInstance()->Error("Couldn't load startup graphic\n");
      dos_logo = NULL;
    }
...
#define MIN(a, b) (a < b ? a : b)
  ...
  MACRO_LOG_DEBUG("Configure buttons.")
  ...
  MACRO_LOG_DEBUG("Loading mission maps")
  missionList = new MissionMapsClass();

  // Load multiPlayer maps
  MACRO_LOG_DEBUG("Loading multi-players maps")
...
void Menu::HandleInput()
{
   ...
   */
  void Menu::loadPal(const std::string& paln, SDL_Color *palette)
  {
