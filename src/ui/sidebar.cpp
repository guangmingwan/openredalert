...
radaranimating(false), unitoff(0), structoff(0), player(pl), scaleq(-1)
{
  ...
  try {
    ...
    throw std::runtime_error("Unable to load the tab images!");
  }
  ...
  try {
    ...
    throw std::runtime_error("Unable to load the powerbar_up images!");
  }
  ...
  try {
    ...
    throw std::runtime_error("Unable to load the power image!");
  }
...
std::string Sidebar::getButtonName(unsigned int index) {
  Uint8* offptr;
...
void Sidebar::DrawButtonTooltip(Uint8 index)
{
  ...
  std::string UnitOrStructureName = getButtonName(index);
  ...
  std::string shortName = UnitOrStructureName.substr(0, 4);
  ...
  std::stringstream TipString;
  // Get raw name of the type
  std::string nameOfType = type->getName();
 ...
 */
void Sidebar::ClickButton(Uint8 index, std::string& unitname, createmode_t* createmode)
{
...
void Sidebar::Build(Uint8 index, Uint8 type, std::string& unitname, createmode_t* createmode) {
  Uint8* offptr;
  std::vector<std::string>* vecptr;
...
void Sidebar::UpdateAvailableLists() {
  std::vector<std::string> units_avail; // list of units available (temp)
  std::vector<std::string> structs_avail; // list of structures available (temp)
  std::vector<std::string> superWeapons_avail; // list of superWeapons available (temp)
  ...
  for (unsigned int i=0;i<units_avail.size();++i) {
    std::string nametemp = units_avail[i] + "ICON.SHP";
  ...
  for (unsigned int i=0;i<structs_avail.size();++i) {
    std::string nametemp = structs_avail[i] + "ICON.SHP";
  ...
  for (unsigned int i=0; i<superWeapons_avail.size(); ++i) {
    std::string nametemp = superWeapons_avail[i] + "ICON.SHP";
    uniticons.push_back(nametemp.c_str());
  }
...
void Sidebar::DrawButton(Uint8 index)
{
  ...
  std::vector<std::string>& icons = ((func&sbo_unit) ? (uniticons) :(structicons));
  ...
  std::string name = icons[offset].substr(length);
