...
CursorPool::CursorPool() {
  uint16_t index = 0; // Index of the cursor info being created
  CursorInfo* data = 0; // Ref to the cursor being created
  std::string cname; // Name of the cursor being created

  // Populate the pool with RA cursors
  //	"standard" normal          0
  index = static_cast<uint16_t>(cursorpool.size());
  data = new CursorInfo();
  data->setAnStart(0);
  data->setAnEnd(0);
  cname = "standard";
...
CursorPool::~CursorPool() {
  ...
  for (uint32_t i = 0; i < cursorpool.size(); ++i) {
    delete cursorpool[i];
  }
}
 ...
 */
CursorInfo* CursorPool::getCursorByName(std::string cname) {
  std::map<std::string, uint16_t>::iterator cursorentry;
  CursorInfo* datum = 0; // Ref to the CursorInfo wanted
  uint16_t index; // Index of the cursor ref if found
