...
void Font::Load(std::string FontName) {
  ...
  if (0 == fontfile) {
    std::string s("Unable to load font ");
    s += fontname;
  ...
  std::vector<Uint8> wchar(nchars);
  std::vector<Uint8> hchar(nchars<<1);

  std::vector<Uint16> dataoffsets(nchars);
  ...
  std::vector<Uint8> chardata(fnheight*fntotalw);
