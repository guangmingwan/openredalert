...
bool Selection::saveSelection(uint8_t savepos) {
  if (savepos > 10)
    return false;
  if (enemy_selected)
    return false;
...
bool Selection::loadSelection(uint8_t savepos) {
  if (savepos > 10) {
    return false;
  }
  clearSelection();
 ...
 */
uint8_t Selection::getOwner() const {
  if (sel_units.empty()) {
...
uint32_t Selection::numbUnits() const {
  return (uint32_t)sel_units.size();
}
