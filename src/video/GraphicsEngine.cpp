...
GraphicsEngine::GraphicsEngine()
{
  ...
  std::string caption = "OpenRedAlert - ";
  caption += VERSION;
...
void GraphicsEngine::setupCurrentGame()
{
  ...
  minizoom.normal = (tilewidth*pc::sidebar->getTabLocation()->w)/std::max(maparea.w, maparea.h);
  minizoom.max    = std::max(1,tilewidth*pc::sidebar->getTabLocation()->w / std::max(
...
void GraphicsEngine::DrawSelectionBox ()
{
  ...
  if (Input::isDrawing()) {
    dest.x = std::min(Input::getMarkRect().x, (Sint16)Input::getMarkRect().w);
    dest.y = std::min(Input::getMarkRect().y, (Sint16)Input::getMarkRect().h);
...
void GraphicsEngine::DrawMinimap()
{
  ...
  if (lplayer->getNumberRadars()>0 && !pc::sidebar->isRadaranimating())
  {
    ...
    minx = std::min(curpos%mapwidth, mapwidth - clip.tilew);
    cury = std::min(curpos/mapwidth, mapheight - clip.tileh);
...
void GraphicsEngine::DrawFogOfWar(SDL_Rect dest, SDL_Rect src, SDL_Rect udest) {
  ...
  mapWidth = std::min(mapWidth, p::ccmap->getWidth());
  mapHeight = (maparea.h+p::ccmap->getYTileScroll()+tilewidth-1)/tilewidth;
  mapHeight = std::min(mapHeight, p::ccmap->getHeight());
...
void GraphicsEngine::DrawMap(SDL_Rect dest, SDL_Rect src, SDL_Rect udest)
{
  ...
  xmax = std::min(p::ccmap->getWidth()-(p::ccmap->getXScroll()+mapWidth), 4);
  ymax = std::min(p::ccmap->getHeight()-(p::ccmap->getYScroll()+mapHeight), 4);
...
void GraphicsEngine::drawLine(Sint16 startx, Sint16 starty,
                              Sint16 stopx, Sint16 stopy, Uint16 width,
                              Uint32 colour) {
